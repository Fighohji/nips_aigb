...
class PlayerBiddingStrategy(BaseBiddingStrategy):
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        ## TODO: 对出价大于历史最低的做一个贪心

        bucket = [i for i in range(len(bids))]  ## 可能选择的机会的下标
        bucket = sorted(bucket, key=lambda x : -history_conversion[x] / historyLeastWinningCost[x] + 1e-4)

        bucket = [i for i in bucket if bids[i] >= historyLeastWinningCost[i]]
        ...
        for i in range(len(bucket) - 1,-1, -1):
            if tot > self.remaining_budget:
                tot -= bids[bucket[i]]
                bids[bucket[i]] = 0
        # all_prices = [price for sublist in historyLeastWinningCost for price in sublist]
        # historical_LeastWinningCost_std = np.std(all_prices) if all_prices else 0

        # expected_costs = 

        # expected_remaining_buget = self.remaining_budget - 
